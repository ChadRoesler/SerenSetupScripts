...
#    2. Makes a clean venv at ~/seren-venvs/memory  (Seren convention)
...
#    5. Drops a run-seren-memory.sh launcher
#    6. (optional) installs an autostart service via setup-memory-service.sh
#
...
#
#  USAGE
#    bash seren-memory-setup.sh                 # easy mode, PyPI, local-only
#    bash seren-memory-setup.sh --gen-token     # generate a bearer token
#    bash seren-memory-setup.sh --service       # + autostart (sudo on linux)
#    bash seren-memory-setup.sh --wheel ./seren_memory-0.1.0-py3-none-any.whl
#    bash seren-memory-setup.sh --ref v0.4.0    # pin to a GitHub release tag
#    bash seren-memory-setup.sh --host 0.0.0.0  # expose on the LAN (careful!)
#    bash seren-memory-setup.sh --mcp           # install the [mcp] extra
#    bash seren-memory-setup.sh --corp          # OS trust store (corp proxy)
#
#  FLAGS
...
#    --service        Autostart via setup-memory-service.sh (systemd/launchd)
#    --mcp            Install with the MCP route (/mcp) and dependencies
...
PY
  ...
  WHEEL_SRC="seren-memory"  # latest from PyPI
...
# Build the extras suffix: [mcp], [corp], or [mcp,corp] (or none).
EXTRAS=""
if $MCP && $CORP;  then EXTRAS="[mcp,corp]"
elif $MCP;         then EXTRAS="[mcp]"
elif $CORP;        then EXTRAS="[corp]"
fi
CORP_ARGS="$(pip_corp_args)"
step "Installing seren-memory${EXTRAS}  (this pulls chromadb$( $MCP && echo " + the MCP SDK" )$( $CORP && echo " + truststore" ))"
...
if $INSTALL_SERVICE; then
  ...
  WRAPPER="$SCRIPT_DIR/setup-memory-service.sh"
  CORE="$(find_upward "Generics/setup-seren-service.sh")"
  if [[ -f "$WRAPPER" && -f "$CORE" ]]; then
    if [[ -n "$TOKEN" ]]; then
      printf 'SEREN_MEMORY_BEARER_TOKEN=%s\n' "$TOKEN" > "$APP_DIR/seren-memory.env"
      chmod 600 "$APP_DIR/seren-memory.env"
    fi
    bash "$WRAPPER" --instance "$INSTANCE" || die "service install failed - see output above"
  else
    warn "setup-memory-service.sh + setup-seren-service.sh not found in $SCRIPT_DIR."
    warn "Keep the shared setup scripts together and run:"
    warn "  bash setup-memory-service.sh --instance '$INSTANCE'"
  fi
fi
...
if ! $INSTALL_SERVICE; then
  ...
  echo -e "  (or directly:    ${B}$VPY -m seren_memory --config $CFG_PATH${NC})"
fi
